...
def main():
    ...
    # Use local trained model instead of downloading
    local_model_path = "models/output/checkpoint-916"
    config = BertConfig.from_pretrained(local_model_path, num_labels=2)
    ...
    class_weight = torch.tensor([1.0, 4.0]).to(model.device)  # TSG = 0, ONC = 1
    ...
    model.compute_loss = custom_loss
